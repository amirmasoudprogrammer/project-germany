...
const steps = [
    {id: 1, icon: <CiCircleCheck size={24}/>, label: "Your selection",},
    {id: 2, icon: <CiShoppingCart size={24}/>, label: "Shopping cart", },
    {id: 3, icon: <CiCreditCard1 size={24}/>, label: "Customer cart", },
    {id: 4, icon: <FaTruck size={22}/>, label: "Order data", },
    {id: 5, icon: <CiCreditCard1 size={24}/>, label: "Payment details", },
];
                    ...
                    {steps.map((item, index) => {
                        ...
                        return (
                            <div key={item.id} className="flex flex-col items-center cursor-pointer group">
                                <motion.div className={`rounded-full shadow-md flex items-center justify-center w-14 h-14 transition-all ${
                                        isActive ? "bg-[#E0F7FD]" : "bg-white"}`} animate={{scale: isCurrent ? 1.2 : 1, boxShadow: isActive ? "0 0 12px rgba(44, 190, 227, 0.4)" : "0 2px 6px rgba(0,0,0,0.1)",}}>
                                   <span className={`transition-colors text-xl ${isActive ? "text-[#2CBEE3]" : "text-gray-400"}`}>
                  {item.icon}
                </span>
                                </motion.div>
                                <span
